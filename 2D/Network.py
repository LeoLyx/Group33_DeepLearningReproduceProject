...
#The TA said we can just only use the averaging kernel and leave Gaussian alone!!
...
class AveragingKernel(torch.nn.Module):
    ...
    def window_averaging(self, v):
        ...
        pad = [half_win + 1, half_win] * 2  # changing the padding's dimension to 2

        v_padded = F.pad(v, pad=pad, mode='constant', value=0)  # [x+pad, y+pad]

        # Run the cumulative sum across x and y dimensions
...
#change the net to be 2D!!!!first thing to do is changing the nn.Conv3d to nn,Conv2d
class BrainNet(ODEF):
    def __init__(self, img_sz, smoothing_kernel, smoothing_win, smoothing_pass, ds, bs):
        ...
        #this part also change to 2d by deleting the img_sz[2] , which representes the z dimension
        ...
        if self.smoothing_kernel == 'AK':
            ...
            #  we can use a built-in function as well!!!
        else:
            self.sk = GaussianKernel(win=smoothing_win, nsig=0.1)
#for the forward part, u also need to change it to 2D!~
    def forward(self, x):
#first for now u dont have the img_sz[2], so delete it!!
        imgx = self.img_sz[0]
        imgy = self.img_sz[1]
        ...
        #         x = self.sk(x)
        #     else:
        #         #i also change this part, but it doesnt matter since we dont use the Guassian!!!
