...
def main(config):
    ...
    print(fixed.shape)#i dont know why our 2D data image is in shape of (160,192,1)?????
    fixed=np.squeeze(fixed, axis=2)#so i have to squeeze the third dim!
    moving = load_nii(config.moving)
    moving=np.squeeze(moving, axis=2)
    ...
    #visualize the grid !!!!!
    visual_df_grid=df_with_grid.cpu().numpy()
    
    fig, ax = plt.subplots()

    x =visual_df_grid[0,:,:,0]

    y = visual_df_grid[0,:,:,1]

    ax.plot(x, y, '-k', linewidth=0.5,color='blue')

    ax.plot(y, x, '-k', linewidth=0.5,color='blue')

    plt.show()
    # the jacobian determinants of the transformation
    jetd_of_grid=JacboianDet(df_with_grid)

    # Only keep negative jacobian determinants 
    neg_jetd_of_grid=torch.where(jetd_of_grid< 0, jetd_of_grid, torch.zeros_like(jetd_of_grid)).square()
    neg_jetd_of_grid=torch.mul(neg_jetd_of_grid,20)
    
    



    #save more imgaes for figure4!!
    save_result(config, df, warped_moving,jetd_of_grid,neg_jetd_of_grid)
...
def registration(config, device, moving, fixed):
    ...
    moving = torch.from_numpy(moving).to("cpu").float()
    fixed = torch.from_numpy(fixed).to("cpu").float()
...
def evaluation(config, device, df, df_with_grid):
    ...
    fixed_seg=np.squeeze(fixed_seg, axis=2)
    moving_seg = load_nii(config.moving_seg)
    moving_seg=np.squeeze(moving_seg, axis=2)
    ST_seg = SpatialTransformer(fixed_seg.shape, mode='nearest').to("cpu")
    moving_seg = torch.from_numpy(moving_seg).to("cpu").float()
    # make batch dimension
...
def save_result(config, df, warped_moving,jetd_of_grid,neg_jetd_of_grid):
    #remember when u save it, also delete the z dimension !!!(2301)
    ...
    save_nii(jetd_of_grid.detach().cpu().numpy(), '%s/jetd.nii.gz' % (config.savepath))
    save_nii(neg_jetd_of_grid.detach().cpu().numpy(), '%s/neg_jetd.nii.gz' % (config.savepath))
